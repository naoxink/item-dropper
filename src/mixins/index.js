...
export default {
  methods: {
    ...
    golpearMonstruoActivo() {
      ...
      const result = this.jugador.atacar(this.enemigo)
      if (result.finished) {
        // Sumar créditos
        this.$store.commit('incrementarCreditoTotal', result.creditos)
        // Generar loot
        this.nuevoDrop()
        this.aumentarAtributosBase()
        if (this.enemigo.esJefe) {
  ...
  golpeaJugador() {
    ...
    const result = this.enemigo.atacar(this.jugador)
    console.log(result)
    // // Defensa
  ...
  monstruoActivoAtaque() {
    const _v = this
    _v.enemigo.atacando = true
    setTimeout(function () {
      _v.golpeaJugador()
      _v.enemigo.atacando = false
    }, this.random(1, 1000))
  },
  aumentarAtributosBase() {
    this.jugador.estadisticas.aumentarVidaInicial(this.jugador.totalEstadistica('vida ') * 1.1)
    this.jugador.estadisticas.aumentarAtaqueInicial(this.jugador.totalEstadistica('atq ') * 1.1)
    this.jugador.estadisticas.aumentarDefensaInicial(this.jugador.totalEstadistica('def ') * 1.1)
  ...
  total(objeto, campo) {
    return objeto.totalEstadistica(campo) + (this.equipado ? this.equipado.estadisticas[campo] : 0)
  },
  ...
  generaObjeto(desdeTienda = false) {
    ...
    if (nivelDesde > 6 && !this.enemigo.esJefe) {
      nivelDesde -= 2
    }
