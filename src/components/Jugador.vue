<template>
  <div class="player">
    ...
    <p><strong>Créditos</strong>: {{ formatoNumero(this.creditoTotal) }}¢ <button @click="alternarTienda()">Mostrar tienda</button></p>
    ...
    <p>
      <strong>Vida</strong>: {{ formatoNumero(jugador.totalEstadistica('vida')) }}
      | <strong>Ataque</strong>: {{ formatoNumero(jugador.totalEstadistica('atq')) }}
      | <strong>Defensa</strong>: {{ formatoNumero(jugador.totalEstadistica('def')) }}
    </p>
    <div class="progress-bar-container vida">
      <div class="progress-bar" :style="'width: ' + porcentageVidaJugador + '%'"></div>
      <div class="progress-bar-extra" :style="'width: ' + porcentageVidaJugadorExtra + '%'"></div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    computed: {
      ...
      porcentageVidaJugador(){
        let totalInicial = this.jugador.totalEstadistica('vida')
        const porcent = Math.round(this.jugador.totalEstadisticaInicial('vida') * 100 / totalInicial)
